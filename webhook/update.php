...
if(preg_match("/\/(dm-.+).php$/",$_SERVER['REQUEST_URI'],$matches))
{
        $service = $matches[1];
}
$logfile = '/data/webhook-logs/' . $service .  '.log';
define('LOGFILE',$logfile);
log_msg("Updating service {$service}");
// what command to execute upon retrieval of a valid push event
$cmd = 'sh && cd /opt/sgov-data-management && ./redeploy.sh ' . $service . ' >' . $logfile . ' 2>&1';
...
$required_data = array(
#        'repository' => array(
#                'full_name' => 'opendata-mvcr/sgov',
#        ),
);
// required data in headers - probably doesn't need changing
$required_headers = array(
        'REQUEST_METHOD' => 'POST',
        'HTTP_X_GITHUB_EVENT' => 'registry_package',
